/**
 * @file common\access_control\serial\inc\fullduplex_serial_media_access_control.h
 *
 * Copyright (C) 2022
 *
 * fullduplex_serial_media_access_control.h is free software: you can redistribute it and/or modify
 ...
 * @encoding utf-8
 */
#ifndef __FULLDUPLEX_SERIAL_MEDIA_ACCESS_CONTROL_H
#define __FULLDUPLEX_SERIAL_MEDIA_ACCESS_CONTROL_H
...
/*---------- function prototype ----------*/
/**
 * @brief Create a new full duplex serial media access controller, and return a handle by which the
 ...
 */
extern serial_mac_t fullduplex_serial_media_access_controller_new(uint32_t baudrate, uint32_t recv_capacity,
        uint32_t trans_capacity, struct serial_mac_ops *ops);

/**
 * @brief Delete a controller that was previously created using the fullduplex_serial_media_access_controller_new()
 * API function.
 * @param self The handle of the controller being deleted.
 * 
 * @retval None
 */
extern void fullduplex_serial_media_access_control_delete(serial_mac_t self);
 ...
 * @retval None
 */
extern void fullduplex_serial_mac_set_transmitter(serial_mac_t self, const uint8_t *pbuf, uint32_t length);

/**
 * @brief Put a series of data to controller's transmitter. The API different from fullduplex_serial_mac_set_transmitter(),
 * it cached data and will try to re-transport if @retrans_count is not zero and 
 * fullduplex_serial_mac_clear_transmitter() not be called during wait ack period.
 ...
 * @param wait_ack_ticks Maximum ticks to wait a ack packet.
 * 
 ...
 */
extern serial_mac_expection_t fullduplex_serial_mac_set_transmitter_cache(serial_mac_t self, const uint8_t *pbuf,
 ...
 * @retval None
 */
extern void fullduplex_serial_mac_clear_transmitter(serial_mac_t self);

/**
 * @brief When recv a byte by serial hardware, fullduplex_serial_mac_recv_byte() API should be called once.
 * @note API fullduplex_serial_mac_recv_byte() should be regiter to serial hardware interrupt receive server function.
 * @param self The handle of controller.
 * @param byte A byte recevied by serial hardware.
 * 
 * @retval None
 */
extern void fullduplex_serial_mac_recv_byte(serial_mac_t self, uint8_t byte);

/**
 * @brief When timer expired, fullduplex_serial_mac_timer_expired() should be called once.
 * @note API fullduplex_serial_mac_timer_expired() should be register to timer hardware interrupt server function.
 * @param self The handle of controller.
 * 
 * @retval None
 */
extern void fullduplex_serial_mac_timer_expired(serial_mac_t self);
 ...
 * @retval None
 */
extern void fullduplex_serial_mac_called_per_tick(serial_mac_t self);

#ifdef __cplusplus
}
#endif
#endif /* __FULLDUPLEX_SERIAL_MEDIA_ACCESS_CONTROL_H */
